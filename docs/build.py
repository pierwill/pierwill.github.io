#!/usr/bin/env python3
"""
Static site builder for pierwill.github.io
...
class StaticSiteBuilder:
    ...
    def create_post_html(self, title, date, content, slug):
        ...
        if template_path.exists():
            ...
            html = template.replace("{{PAGE_TITLE}}", f"{title} | pierwill")
            html = html.replace("{{PAGE_DESCRIPTION}}", title)
            html = html.replace("{{CANONICAL_URL}}", f"https://www.pierwill.com/post/{slug}/")
...
<body>
    <div class="container">
        ...
        <footer class="footer">
            <div class="site-info">© 1969–2025 pierwill</div>
        </footer>
    </div>
</body>
</html>"""
    ...
    def create_homepage_html(self, posts_html, pagination_html, page_num=None):
        ...
        if template_path.exists():
            ...
            if page_num and page_num > 1:
                title = f"pierwill - Page {page_num}"
            ...
            html = html.replace("{{CANONICAL_URL}}", "https://www.pierwill.com/")
...
<body>
    ...
    def generate_archive_page(self):
        ...
        if template_path.exists():
            ...
            html = template.replace("{{PAGE_TITLE}}", "Archive | pierwill")
            html = html.replace("{{PAGE_DESCRIPTION}}", "Complete archive of all blog posts")
            html = html.replace("{{CANONICAL_URL}}", "https://www.pierwill.com/archive/")
            html = html.replace("{{OG_TYPE}}", "website")
            html = html.replace("{{TWITTER_CARD}}", "summary")
    ...
    def generate_about_page(self):
        ...
        if about_md_path.exists():
            ...
            if template_path.exists():
                ...
                html = template.replace("{{PAGE_TITLE}}", "About | pierwill")
                html = html.replace("{{PAGE_DESCRIPTION}}", "About pierwill")
                html = html.replace("{{CANONICAL_URL}}", "https://www.pierwill.com/about/")
                html = html.replace("{{OG_TYPE}}", "website")
                html = html.replace("{{TWITTER_CARD}}", "summary")
                html = html.replace("{{SCHEMA_TYPE}}", "WebPage")
                html = html.replace("{{PUBLISH_DATE}}", "2025-02-15T00:00:00+00:00")
